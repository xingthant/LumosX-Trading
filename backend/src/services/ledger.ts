...
/** Moves funds from available_balance into locked_balance (order placement). */
export async function lockFunds(client: PoolClient, userId: string, asset: string, amount: number) {
  ...
  const res = await client.query(
    `UPDATE user_balances
     SET available_balance = available_balance - $3,
         ...
         updated_at = now()
     WHERE user_id = $1 AND asset_symbol = $2 AND available_balance >= $3
     RETURNING *`,
    [userId, asset, amount]
  );
  if (res.rowCount === 0) {
...
/** Returns previously locked funds back to available_balance (order cancel / withdrawal rejection). */
...
/** Permanently deducts from locked_balance (order fill on the spent side, withdrawal approval). */
...
/** Credits available_balance directly (order fill on the received side, deposit, admin credit). */
...
/** Debits available_balance directly (admin debit). Throws if insufficient. */
export async function debitAvailable(client: PoolClient, userId: string, asset: string, amount: number) {
  await ensureBalanceRow(client, userId, asset);
  const res = await client.query(
    `UPDATE user_balances
     SET available_balance = available_balance - $3,
...
export async function getBalances(userId: string) {
  return withTransaction(async (client) => {
    const res = await client.query(
      `SELECT asset_symbol, available_balance, locked_balance, updated_at
       FROM user_balances WHERE user_id = $1 ORDER BY asset_symbol`,
      [userId]
    );
    return res.rows;
  });
